...
function calcSLTP(sym, entryPrice) {
  ...
  return { slPrice, tpNote: 'Exit when spike candle closes' };
}
...
function buildSignalMsg(sym, prob, ticksLeft, entryPrice, sl, autoTrade) {
  ...
  return (
    ...
    `${autoTrade ? '🤖 <b>Auto-trade sent to MT5</b>' : '⚠️ <b>Manual trade needed</b>'}\n` +
...
function buildSpikeConfirmMsg(sym, move, avgMove, ticksAtSpike, entryPrice, hadSignal) {
  ...
  const note    = hadSignal ? `<i>✅ Closing trade on MT5 now.</i>` : `<i>⚠️ No signal this cycle — spike at tick ${ticksAtSpike}.</i>`;
...
function buildDailySummary() {
  ...
  const symRows = Object.keys(CONFIG.SYMBOLS).map(sym => {
    ...
    return `  ${s.label}: ${r.wins}W/${r.losses}L  WR:${wr}  ${r.pnl>=0?'+':''}$${r.pnl.toFixed(2)}`;
  }).join('\n');
  return (
    ...
    `${session.worstTrade ? `💔 Worst: -$${Math.abs(session.worstTrade.pnl).toFixed(2)} (${CONFIG.SYMBOLS[session.worstTrade.sym].label})` : '💔 Worst: —'}`
  );
}
...
function processTick(sym, price) {
  ...
  // ── WARNING ──────────────────────────────────────────────────────
  if (ticksLeft <= CONFIG.WARNING_TICKS_OUT && ticksLeft > CONFIG.SIGNAL_TICKS_OUT && !st.warnFired && cooldownOk) {
    st.warnFired = true;
  ...
  // ── SPIKE DETECTION (exact v2.0 — 6x multiplier) ─────────────────
  ...
  if (st.ticks > st.nextSpike * 1.5) {
    console.log(`[RESET] ${sym} — passed window`);
    _resetState(sym);
  }
}
...
function startHeartbeat() {
  setInterval(() => {
    ...
    Object.keys(CONFIG.SYMBOLS).forEach(sym => {
      ...
      console.log(`${st.connected?'🟢':'🔴'} ${s.label.padEnd(12)} | ticks: ${String(st.ticks).padStart(4)}/${st.nextSpike} | RSI: ${String(st.rsi).padStart(5)} | prob: ${String(calcProbability(sym)).padStart(4)}%`);
    });
...
console.log('  v2.0 logic + auto-trade + daily summary');
console.log('════════════════════════════════════════\n');
