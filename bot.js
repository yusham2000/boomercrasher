...
// ═══════════════════════════════════════════════════════════════════
//  BOOM & CRASH ALL-IN-ONE BOT — v3.0
//  Detects spikes + Places trades directly on Deriv + Telegram alerts
//  No MT5, no ngrok, no listener — everything in one bot
// ═══════════════════════════════════════════════════════════════════
...
const CONFIG = {
  ...
  SYMBOLS: {
    ...
    'BOOM500':   { label: 'Boom 500',   type: 'boom',  period: 500,  direction: 'UP 📈',   stake: 0.35, contract: 'CALL' },
    'CRASH1000': { label: 'Crash 1000', type: 'crash', period: 1000, direction: 'DOWN 📉', stake: 0.35, contract: 'PUT'  },
    'CRASH500':  { label: 'Crash 500',  type: 'crash', period: 500,  direction: 'DOWN 📉', stake: 0.35, contract: 'PUT'  },
  },

  STAKE:                   0.35,   // $ stake per trade (minimum on Deriv)
  SIGNAL_TICKS_OUT:        20,
...
function connectTradeWs() {
  if (!CONFIG.DERIV_API_TOKEN) {
    console.log('[TRADE] No API token — trading disabled');
    return;
  }
  ...
  tradeWs.on('message', raw => {
    try {
      ...
      if (msg.msg_type === 'authorize') {
        ...
        console.log(`[TRADE] Authorized | Balance: ${bal} ${msg.authorize.currency}`);
        sendTelegram(`✅ <b>Deriv trading authorized</b>\nBalance: ${bal} ${msg.authorize.currency}`);
      ...
      if (msg.msg_type === 'buy') {
        ...
        const c = msg.buy;
        console.log(`[TRADE] Opened | ID=${c.contract_id} | buy_price=${c.buy_price}`);

        // Find which symbol this belongs to and store contract ID
      ...
      if (msg.msg_type === 'sell') {
        if (msg.error) {
          console.error('[TRADE] Sell error:', msg.error.message);
          return;
        }
        const sold = msg.sell;
...
function placeTrade(sym, entryPrice) {
  ...
  // Use TICKSHIGH/TICKSLOW — best for spike trading
  // TICKSHIGH = price will touch a high tick (boom)
  // TICKSLOW  = price will touch a low tick (crash)
  ...
  const proposal = {
    ...
    symbol:        sym,
    duration:      1,
    duration_unit: 't',  // 1 tick contract — exits on next tick
  };

  console.log(`[TRADE] Proposing ${contractType} on ${sym} stake=$${CONFIG.STAKE}`);
...
function processTick(sym, price) {
  ...
  if (st.prices.length >= 2 && st.ticks > 30) {
    ...
    if (move > st.avgMove * CONFIG.SPIKE_DETECT_MULTIPLIER) {
      ...
      const pnl = hadSignal ? parseFloat((move * 0.10).toFixed(2)) : null;
  ...
  if ((tickTrigger || probTrigger) && !st.signalFired && cooldownOk) {
    ...
    // Place trade on Deriv automatically
...
function _resetState(sym) {
  ...
  st.entryPrice = null; st.openContractId = null;
...
// ── Startup ──────────────────────────────────────────────────────────
console.log('════════════════════════════════════════');
console.log('  Boom & Crash All-in-One Bot  v3.0');
console.log('════════════════════════════════════════');
...
sendTelegram(
  `🤖 <b>Boom & Crash All-in-One Bot v3.0 — ONLINE</b>\n` +
  `━━━━━━━━━━━━━━━━━━━━━━\n` +
  `📡 Monitoring: All 4 Boom & Crash indices\n` +
  `🤖 Auto-trade: ${CONFIG.DERIV_API_TOKEN ? '✅ Deriv API connected' : '⚠️ No API token'}\n` +
